...
│   │   │   │   ├── ingestion/→ poll-company, seed scripts
│   │   │   │   ├── llm/      → AI SDK service layer
│   │   │   │   └── search/   → filter pipeline, synonym cache, search types
...
│           ├── geo/          → structured location matching (P6.1)
│           ├── normalizer/   → job normalization, dedup, hashing
│           ├── taxonomy/     → synonym expansion service (P6.2)
...
| ~~**P5**~~ | ~~Chatbot: preferences~~ | `ai`, `@ai-sdk/anthropic`, `react-hook-form`, conversation engine | ✅ Done |
| ~~**P6**~~ | ~~Search API~~ | Instant DB search with Level 2 filter | ✅ Done |
| ~~**P6.1**~~ | ~~Structured location matching~~ | `packages/ats-core/src/geo/` — geo parser, resolver, tier matcher, LRU cache | ✅ Done |
| ~~**P6.2**~~ | ~~Synonym normalization~~ | `synonym_group` DB table, `packages/ats-core/src/taxonomy/`, synonym cache | ✅ Done |
| ~~**P7**~~ | ~~pg-boss worker + Render deploy~~ | `pg-boss`, `apps/worker/`, Render services | ✅ Done |
| ~~**P8**~~ | ~~LLM scoring~~ | Level 3 evaluation, RSLCD scoring, `job_matches` | ✅ Done |
| ~~**P9**~~ | ~~Adaptive polling + rate limiting~~ | Vendor queues, jitter, backoff | ✅ Done |
| ~~**P10**~~ | ~~Internet search expansion~~ | Company discovery, ATS detection, immediate poll | ✅ Done |
| **P10.1** | ATS probe-first detection | Direct API probing, save all companies, `atsSearchLog`, incremental pipeline | P10 |
